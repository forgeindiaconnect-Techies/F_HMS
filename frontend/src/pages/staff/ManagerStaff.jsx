...
const ManagerStaff = () => {
    ...
    const fetchStaff = async () => {
        try {
            ...
            // Filter staff members assigned to this branch, excluding Managers and Admins from attendance
            const filtered = res.data.filter(s => {
                if (s.role === 'BranchManager' || s.role === 'RestaurantAdmin' || s.role === 'SuperAdmin') {
                    return false;
                }
        ...
        <div className="p-8 max-w-[1600px] mx-auto space-y-6 font-sans">
            <div className="flex justify-between items-end mb-6">
                ...
                <div className="flex gap-3">
                    ...
                    <button onClick={() => setShowAssignModal(true)} className="bg-green-600 hover:bg-green-700 text-white px-5 py-2.5 rounded-xl font-bold transition-colors flex items-center gap-2 text-sm shadow-md cursor-pointer">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-gray-200 dark:border-slate-800 shadow-sm flex items-center gap-4">
                    <div className="p-3 bg-green-50 dark:bg-green-950/40 text-green-600 dark:text-green-400 rounded-lg"><UserCheck size={20} /></div>
                    <div><p className="text-xs text-gray-500 dark:text-slate-400 font-bold uppercase">Clocked In</p><h3 className="text-xl font-bold text-gray-900 dark:text-white">{clockedInCount}</h3></div>
                </div>
                <div className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-gray-200 dark:border-slate-800 shadow-sm flex items-center gap-4">
                    <div className="p-3 bg-orange-50 dark:bg-orange-950/40 text-orange-600 dark:text-orange-400 rounded-lg"><Clock size={20} /></div>
                    <div><p className="text-xs text-gray-500 dark:text-slate-400 font-bold uppercase">On Break</p><h3 className="text-xl font-bold text-gray-900 dark:text-white">{onBreakCount}</h3></div>
                </div>
                <div className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-gray-200 dark:border-slate-800 shadow-sm flex items-center gap-4">
                    <div className="p-3 bg-red-50 dark:bg-red-950/40 text-red-600 dark:text-red-400 rounded-lg"><UserX size={20} /></div>
                    <div><p className="text-xs text-gray-500 dark:text-slate-400 font-bold uppercase">Late / Absent</p><h3 className="text-xl font-bold text-gray-900 dark:text-white">{absentCount}</h3></div>
                </div>
                <div className="bg-gradient-to-r from-gray-900 to-gray-800 dark:from-slate-900 dark:to-slate-950 p-4 rounded-xl shadow-sm text-white border border-transparent dark:border-slate-800 flex items-center justify-between">
                    <div>
                        <p className="text-xs text-gray-400 dark:text-slate-400 font-bold uppercase">Labor Cost (Today)</p>
                        <h3 className="text-xl font-bold mt-1">₹{laborCostToday}</h3>
                    </div>
                    <div className="text-right">
                        <p className="text-xs text-green-400 font-bold">Optimal</p>
                        <p className="text-[10px] text-gray-400 dark:text-slate-400 mt-1">18% of Rev</p>
                    </div>
                </div>
            </div>
            ...
            <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-800 overflow-hidden">
                <div className="p-5 border-b border-gray-100 dark:border-slate-800 flex justify-between items-center bg-gray-50/50 dark:bg-slate-950/50">
                    <h3 className="font-bold text-gray-900 dark:text-white text-lg" style={{ fontFamily: 'Poppins, sans-serif' }}>Current Shift Roster</h3>
                    <div className="relative">
                        ...
                        <input 
                            ...
                            className="pl-9 pr-4 py-1.5 bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 text-gray-900 dark:text-white rounded-lg text-sm focus:outline-none focus:border-green-500" 
                        />
                    </div>
                </div>
                    ...
                    <table className="w-full text-left whitespace-nowrap">
                        <thead className="bg-gray-50/50 dark:bg-slate-950/50 border-b border-gray-100 dark:border-slate-800">
                            <tr>
                                ...
                                <th className="px-6 py-4 text-xs font-bold text-gray-500 dark:text-slate-400 uppercase tracking-wider">Time In</th>
                                <th className="px-6 py-4 text-xs font-bold text-gray-500 dark:text-slate-400 uppercase tracking-wider">Hours Today</th>
                        ...
                        <tbody className="divide-y divide-gray-100 dark:divide-slate-800">
                            {loading ? (
                                ...
                                <tr>
                                    <td colSpan="5" className="text-center py-8 text-gray-500 dark:text-slate-400 font-medium">
                                    ...
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-xs text-gray-500 dark:text-slate-400">{staff.role.replace(/([A-Z])/g, ' $1').trim()}</p>
                                            </div>
                                        </div>
                                    </td>
                                    <td className="px-6 py-4">
                                        <span className={`px-2.5 py-1 rounded text-[10px] font-bold uppercase tracking-wider ${
                                            staff.status === 'Active' ? 'bg-green-100 dark:bg-green-950/60 text-green-700 dark:text-green-400' :
                                            staff.status === 'Break' ? 'bg-orange-100 dark:bg-orange-950/60 text-orange-700 dark:text-orange-400' :
                                            'bg-red-100 dark:bg-red-950/60 text-red-700 dark:text-red-400'
                                        }`}>
                                            {staff.status}
                                        </span>
                                    </td>
                                    <td className="px-6 py-4 text-sm font-medium text-gray-700 dark:text-slate-300">{staff.timeIn}</td>
                                    <td className="px-6 py-4 text-sm font-bold text-gray-900 dark:text-white">{staff.hours}</td>
                                    <td className="px-6 py-4 text-right">
                                        <div className="flex justify-end gap-2">
                                            <button onClick={() => setSelectedStaffForManage(staff)} className="text-sm font-bold text-gray-700 dark:text-slate-200 bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 px-3 py-1.5 rounded-lg hover:bg-gray-50 dark:hover:bg-slate-700 transition-colors shadow-sm cursor-pointer">
                                                Manage
                                            </button>
                                        </div>
                                    </td>
