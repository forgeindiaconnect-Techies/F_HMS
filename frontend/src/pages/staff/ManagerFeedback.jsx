...
const ManagerFeedback = () => {
    ...
    const fetchFeedback = async () => {
        try {
            ...
            // Filter completed orders that have feedback ratings
            const reviewed = res.data.filter(order => 
                (order.deliveryRating && (order.deliveryRating.overall > 0 || order.deliveryRating.review)) ||
                (order.internalRating && order.internalRating > 0)
            ).map(order => {
                const rating = order.deliveryRating?.overall || order.internalRating || 5;
                const comment = order.deliveryRating?.review || 'No comment provided.';
                const type = rating >= 4 ? 'Positive' : rating <= 2 ? 'Negative' : 'Neutral';
                const status = rating <= 2 ? 'Requires Action' : 'New';
                return {
    ...
    const filteredFeedback = feedback.filter(fb => {
        ...
        if (filterType === 'Requires Action') return fb.status === 'Requires Action';
        if (filterType === '5 Stars') return fb.rating === 5;
        if (filterType === 'Negative') return fb.type === 'Negative';
        return true;
    });
        ...
        <div className="p-8 max-w-[1600px] mx-auto space-y-6 font-sans">
            <div className="flex justify-between items-end mb-6">
                ...
                <div className="flex gap-4 items-center">
                    <div className="text-right">
                        ...
                        <p className="text-2xl font-bold text-gray-900 flex items-center justify-end gap-1">{averageRating} <Star size={20} className="text-yellow-400 fill-current" /></p>
                    </div>
                </div>
            </div>
            ...
            <div className="flex gap-2 mb-6 overflow-x-auto pb-2 custom-scrollbar">
                ...
                {[
                    ...
                    { id: '5 Stars', label: `5 Stars (${feedback.filter(f => f.rating === 5).length})` },
                    { id: 'Negative', label: `Negative (${feedback.filter(f => f.type === 'Negative').length})` }
                ].map((filter) => (
                    <button 
                        key={filter.id} 
                        onClick={() => setFilterType(filter.id)} 
                        className={`px-4 py-2 rounded-xl text-sm font-bold whitespace-nowrap transition-all ${
                            filterType === filter.id ? 'bg-gray-900 text-white' : 'bg-white border border-gray-200 text-gray-600 hover:bg-gray-50'
                        }`}
                    >
                        {filter.label}
                    </button>
            ...
            {loading ? (
                ...
                <div className="bg-white rounded-2xl p-12 border border-gray-100 text-center space-y-3 shadow-sm">
                    <p className="text-gray-400 font-medium">No reviews found matching the filter.</p>
                </div>
            ) : (
