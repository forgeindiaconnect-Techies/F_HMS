...
const WaiterDashboard = () => {
    ...
    const handleGenerateBill = async (tableObj) => {
        ...
        try {
            ...
            // Also notify Cashier via service request notification if needed
            try {
                await api.post('/service-requests', {
                    tableNumber: tableToUse.id || tableToUse.rawId,
                    requestType: 'Bill Request',
                    note: `Bill generated by Waiter for Order #${String(tableToUse.orders._id).substring(String(tableToUse.orders._id).length - 5).toUpperCase()}`
                });
            } catch (srErr) {}

            fetchData();
            toast.success(`🧾 Bill generated for ${tableToUse.id || 'Table'}! Sent to Cashier Dashboard for settlement.`);
        } catch (error) {
    ...
    const handleExecuteTransfer = async () => {
        const effectiveSourceNum = sourceTableId || activeTable?.id?.replace(/^Table\s+/i, '') || activeTable?.rawId;
        const sourceTableObj = tables.find(t => String(t.id).replace(/^Table\s+/i, '') === String(effectiveSourceNum) || String(t.rawId) === String(effectiveSourceNum)) || activeTable;
        const sourceOrder = sourceTableObj?.orders || activeOrders.find(o => String(o.tableNumber).replace(/^Table\s+/i, '') === String(effectiveSourceNum));

        if (!sourceOrder) {
            toast.error("Please select a table with an active order to transfer.");
            return;
        }
        if (!targetTableId) {
            toast.error("Please select a target destination table.");
            return;
        }
        if (String(effectiveSourceNum) === String(targetTableId)) {
            toast.error("Source and target tables cannot be the same.");
            return;
        }

        try {
            await api.put(`/orders/${sourceOrder._id}/status`, { tableNumber: String(targetTableId) });
            
            // Clean local table statuses if dbId present
            if (sourceTableObj?.dbId) {
                await api.put(`/tables/${sourceTableObj.dbId}/status`, { status: 'Available', customers: 0 }).catch(() => {});
            }
            const destTable = dbTables.find(t => String(t.tableNumber) === String(targetTableId));
            if (destTable) {
                await api.put(`/tables/${destTable._id}/status`, { status: 'Occupied', customers: sourceTableObj?.customers || 2 }).catch(() => {});
            }

            setTransferModalOpen(false);
            setPanelOpen(false);
            fetchData();
            toast.success(`Transferred order from Table ${effectiveSourceNum} to Table ${targetTableId}!`);
        } catch (error) {
    ...
    const handleExecuteSplitBill = async () => {
        const effectiveSourceNum = sourceTableId || activeTable?.id?.replace(/^Table\s+/i, '') || activeTable?.rawId;
        const sourceTableObj = tables.find(t => String(t.id).replace(/^Table\s+/i, '') === String(effectiveSourceNum) || String(t.rawId) === String(effectiveSourceNum)) || activeTable;
        const targetOrder = sourceTableObj?.orders || activeOrders.find(o => String(o.tableNumber).replace(/^Table\s+/i, '') === String(effectiveSourceNum));

        if (!targetOrder) {
            toast.error("Please select an occupied table with an active order.");
            return;
        }
        const total = targetOrder.totalPrice || 0;
        const perPerson = (total / splitParts).toFixed(2);

        try {
            await api.put(`/orders/${targetOrder._id}/items`, {
                orderItems: [],
                totalPrice: 0,
                notes: `[Split Bill (${splitParts} Guests): ₹${perPerson} per guest]`
            }).catch(() => {});
        } catch (e) {}

        toast.success(`Bill for Table ${targetOrder.tableNumber || effectiveSourceNum} split into ${splitParts} parts: ₹${perPerson} per guest!`);
        setSplitModalOpen(false);
    };

    const handleExecuteCallManager = async () => {
        const tableNum = activeTable?.id || (sourceTableId ? `Table ${sourceTableId}` : 'Floor');
        try {
            await api.post('/service-requests', {
            ...
            toast.success(`Manager alerted for ${tableNum}: ${managerReason}`);
        } catch (error) {
    ...
    // Calculate split bill details for Split Bill Modal
    const splitEffectiveSourceNum = sourceTableId || activeTable?.id?.replace(/^Table\s+/i, '') || activeTable?.rawId;
    const splitSourceTableObj = tables.find(t => String(t.id).replace(/^Table\s+/i, '') === String(splitEffectiveSourceNum) || String(t.rawId) === String(splitEffectiveSourceNum)) || activeTable;
    const splitOrderToSplit = splitSourceTableObj?.orders || activeOrders.find(o => String(o.tableNumber).replace(/^Table\s+/i, '') === String(splitEffectiveSourceNum));
    const splitBillTotal = splitOrderToSplit?.totalPrice || 0;
    const splitPerGuest = splitParts > 0 ? (splitBillTotal / splitParts).toFixed(2) : 0;
                    ...
                    <div className="absolute bottom-16 right-0 w-56 bg-white rounded-2xl border border-slate-200 shadow-2xl p-2 space-y-1 z-50 animate-in fade-in slide-in-from-bottom-2">
                        ...
                        <button 
                            onClick={() => { 
                                ...
                                setSourceTableId(occupied ? (occupied.rawId || String(occupied.id).replace(/^Table\s+/i, '')) : '');
                                setTargetTableId('');
                ...
                <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-xs z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-3xl border border-slate-200 shadow-2xl max-w-md w-full p-6 space-y-5 animate-in fade-in zoom-in-95">
                        ...
                        <div className="space-y-3">
                            <div>
                                ...
                                >
                                    ...
                                    {tables.filter(t => t.orders).map(t => (
                                        <option key={t.id} value={t.rawId || t.id.replace(/^Table\s+/i, '')}>
                                            {t.id} (Order #{String(t.orders._id).slice(-5).toUpperCase()} - ₹{t.orders.totalPrice})
                                        </option>
                                    ))}
                                </select>
                            </div>
                            ...
                            <div>
                                <label className="text-xs font-bold text-slate-700 block mb-1">Target Table (Destination):</label>
                                <select 
                                    value={targetTableId} 
                        ...
                        <div className="flex justify-end gap-2 pt-2">
                            ...
                            <button onClick={handleExecuteTransfer} className="px-5 py-2 bg-purple-600 hover:bg-purple-700 text-white font-extrabold rounded-xl text-xs shadow-md cursor-pointer">
                                Confirm Transfer
                            </button>
                        </div>
                    </div>
            ...
            {splitModalOpen && (
                <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-xs z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-3xl border border-slate-200 shadow-2xl max-w-md w-full p-6 space-y-5 animate-in fade-in zoom-in-95">
                        ...
                        <div>
                            <label className="text-xs font-bold text-slate-700 block mb-1">Select Occupied Table:</label>
                            <select 
                                value={sourceTableId} 
                                onChange={(e) => setSourceTableId(e.target.value)}
                                className="w-full p-3 rounded-xl border border-slate-200 text-xs font-bold text-slate-800 bg-slate-50 focus:outline-none focus:border-amber-500 mb-3"
                            >
                                <option value="">-- Select Table --</option>
                                {tables.filter(t => t.orders).map(t => (
                                    <option key={t.id} value={t.rawId || t.id.replace(/^Table\s+/i, '')}>
                                        {t.id} (Bill: ₹{t.orders.totalPrice})
                                    </option>
                                ))}
                            </select>
                        </div>
                        ...
                        <div className="bg-amber-50 p-4 rounded-2xl border border-amber-200 text-amber-900 space-y-1">
                            <p className="text-xs font-bold">Bill Total for {splitSourceTableObj?.id || 'Selected Table'}:</p>
                            <p className="text-2xl font-black">₹{splitBillTotal}</p>
                        </div>

                        <div>
                            <label className="text-xs font-bold text-slate-700 block mb-1.5">Number of Guests / Split Parts:</label>
                            <div className="flex items-center gap-3">
                                <button onClick={() => setSplitParts(Math.max(2, splitParts - 1))} className="w-10 h-10 rounded-xl bg-slate-100 text-slate-700 font-black text-base hover:bg-slate-200 cursor-pointer">-</button>
                                <span className="font-extrabold text-lg text-slate-900 w-8 text-center">{splitParts}</span>
                                <button onClick={() => setSplitParts(splitParts + 1)} className="w-10 h-10 rounded-xl bg-amber-100 text-amber-800 font-black text-base hover:bg-amber-200 cursor-pointer">+</button>
                            </div>
                        </div>

                        <div className="p-3 bg-slate-50 rounded-xl text-xs font-bold text-slate-700 flex justify-between">
                            <span>Amount Per Guest:</span>
                            <span className="text-amber-700 font-black">₹{splitPerGuest}</span>
                        </div>
                        ...
                        <div className="flex justify-end gap-2 pt-2">
                            ...
                            <button onClick={handleExecuteSplitBill} className="px-5 py-2 bg-amber-600 hover:bg-amber-700 text-white font-extrabold rounded-xl text-xs shadow-md cursor-pointer">
                                Confirm Split
                            </button>
                        </div>
                    </div>
