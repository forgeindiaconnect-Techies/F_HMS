    ...
    if (loading) {
        return (
            <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center text-white gap-4">
                ...
                <p className="text-xs text-slate-400 font-bold uppercase tracking-widest animate-pulse">Initializing Portal...</p>
            </div>
        );
    }

    return (
        <div className="min-h-screen bg-slate-950 font-sans text-slate-100 pb-28 flex flex-col selection:bg-emerald-500/20 selection:text-emerald-300">
            {/* Header */}
            <header className="bg-slate-900/60 backdrop-blur-md px-6 py-4 border-b border-slate-800/80 flex justify-between items-center sticky top-0 z-20 shadow-[0_4px_30px_rgba(0,0,0,0.4)]">
                <div className="flex items-center gap-3.5">
                    ...
                    <div>
                        <h1 className="text-sm font-black tracking-wide leading-none text-white">{user?.name}</h1>
                        <div className="flex items-center gap-1 mt-1.5">
                            <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-ping"></span>
                            <p className="text-[10px] text-slate-400 font-bold tracking-wider uppercase">Active Agent</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-3">
                    ...
                    >
                        <div className={`w-7 h-7 rounded-full bg-white shadow-md flex items-center justify-center transform transition-transform duration-350 ${
                            ...
                            <Power size={12} className={profile?.status === 'Online' ? 'text-emerald-500' : 'text-slate-400'} />
                        </div>
                        <span className={`absolute text-[9px] font-black uppercase tracking-wider transition-opacity duration-300 ${
                            profile?.status === 'Online' ? 'left-3 text-white' : 'right-3 text-slate-400'
                        }`}>
                            {profile?.status}
                        </span>
                    </button>
                    ...
                    <button 
                        ...
                        className="p-2.5 bg-slate-800/80 hover:bg-red-500/10 border border-slate-700 text-slate-400 hover:text-red-400 rounded-xl transition-all active:scale-95 cursor-pointer"
            ...
            <main className="flex-1 px-6 py-10 w-full max-w-6xl mx-auto space-y-6">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
                        ...
                        <div className="lg:col-span-2 space-y-5">
                            <div className="flex justify-between items-center px-1">
                                <h2 className="text-xs font-black text-slate-400 uppercase tracking-widest">Assigned Deliveries</h2>
                                <span className="text-[10px] bg-slate-900 border border-slate-800 px-2.5 py-0.5 rounded-full text-slate-400 font-extrabold">{assignedOrders.length} Runs</span>
                            </div>
                            ...
                            {assignedOrders.filter(o => o.deliveryStatus === 'Accepted' && (o.status === 'Ready' || o.status === 'Ready for Pickup')).map(order => (
                                <div key={`ready-alert-${order._id}`} className="bg-gradient-to-r from-emerald-500/10 to-teal-500/10 border border-emerald-500/30 p-4.5 rounded-3xl flex items-center gap-4.5 animate-pulse shadow-lg shadow-emerald-500/5">
                                    <span className="relative flex h-3.5 w-3.5 shrink-0">
                                        <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"></span>
                                        <span className="relative inline-flex rounded-full h-3.5 w-3.5 bg-emerald-500"></span>
                                    </span>
                                    <div className="flex-1 text-xs">
                                        <p className="font-extrabold text-emerald-400 uppercase tracking-wider text-[9px] flex items-center gap-1">
                                            <Award size={10} /> Kitchen Alert
                                        </p>
                                        <p className="text-slate-200 mt-1 font-bold">Order <span className="text-emerald-400">#{order._id.substring(order._id.length - 4).toUpperCase()}</span> is fully prepared! Please proceed to pickup.</p>
                                    </div>
                                </div>
                            ))}

                            {assignedOrders.length === 0 ? (
                                <div className="bg-slate-900/60 backdrop-blur-md border border-slate-800/80 p-12 rounded-[2.5rem] text-center text-slate-400 flex flex-col items-center justify-center gap-4 shadow-xl">
                                    <div className="w-16 h-16 rounded-full bg-slate-950 flex items-center justify-center text-slate-700 shadow-inner">
                                        <ListTodo size={32} />
                                    </div>
                                    <div className="space-y-1">
                                        <p className="font-black text-white text-sm">All caught up!</p>
                                        <p className="text-[10px] text-slate-500 max-w-[220px] mx-auto leading-relaxed">Toggle your status to Online to accept new incoming delivery runs.</p>
                                    </div>
                                </div>
                            ) : (
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    {assignedOrders.map(order => (
                                        <div key={order._id} className="bg-slate-900/60 backdrop-blur-md p-6 rounded-[2.5rem] border border-slate-800/80 shadow-[0_10px_30px_rgba(0,0,0,0.5)] hover:border-emerald-500/20 transition-all duration-300 space-y-6">
                                            {/* Order Meta Header */}
                                            <div className="flex justify-between items-start border-b border-slate-800/50 pb-4.5">
                                                <div>
                                                    <div className="flex items-center gap-1.5">
                                                        <span className="w-1.5 h-1.5 rounded-full bg-amber-400"></span>
                                                        <p className="text-[9px] font-black text-slate-500 uppercase tracking-wider">Run Reference</p>
                                                    </div>
                                                    <h3 className="text-lg font-black text-white mt-1">#{order._id.substring(order._id.length - 4).toUpperCase()}</h3>
                                                ...
                                                <div className="text-right">
                                                    <span className="text-[9px] bg-slate-950 text-slate-400 border border-slate-800 px-2.5 py-1 rounded-full font-black uppercase tracking-wider">
                                                        {order.deliveryStatus}
                                                    </span>
                                                    <p className="text-lg font-black text-emerald-400 mt-2">₹{order.deliveryCharge}</p>
                                                </div>
                                            </div>
                                            ...
                                            <div className="relative py-2 flex items-center justify-between text-[10px] font-bold text-slate-500">
                                                ...
                                                {[
                                                    ...
                                                    <div key={idx} className="flex flex-col items-center gap-1 z-10 bg-slate-900 px-1.5 rounded">
                                                        ...
                                                        <span className={`text-[8px] font-extrabold tracking-wide uppercase ${step.active ? 'text-emerald-400' : 'text-slate-500'}`}>{step.name}</span>
                                                    </div>
                                                ))}
                                            </div>

                                            {/* Distance & Info */}
                                            <div className="grid grid-cols-2 gap-4 text-xs font-semibold text-slate-350 pt-1 text-left">
                                                <div className="space-y-1">
                                                    <span className="text-[9px] text-slate-500 uppercase tracking-wider block font-bold">Restaurant Pickup</span>
                                                    <p className="truncate text-white font-extrabold">Hub Kitchen Counter</p>
                                                </div>
                                                <div className="space-y-1 text-right">
                                                    <span className="text-[9px] text-slate-500 uppercase tracking-wider block font-bold">Customer Destination</span>
                                                    <p className="truncate text-white font-extrabold">{order.shippingAddress?.address || 'Customer Location'}</p>
                                                </div>
                                            </div>
                                            ...
                                            <div className="grid grid-cols-3 gap-2 border-t border-slate-800/60 pt-4">
                                                <button 
                                                    ...
                                                    className="py-2.5 bg-slate-800/80 hover:bg-slate-700 text-slate-200 border border-slate-750 rounded-xl flex items-center justify-center gap-1.5 text-xs font-black transition-all active:scale-95 cursor-pointer shadow-sm"
                                                >
                                            ...
                                            <div className="pt-2">
                                                {order.deliveryStatus === 'Pending Assignment' && (
                                                    <div className="flex gap-3">
                                                        ...
                                                        <button
                                                            ...
                                                            className="flex-1 py-3.5 bg-red-500/10 hover:bg-red-500/20 text-red-400 border border-red-500/20 rounded-2xl text-xs font-black tracking-wider uppercase transition-all cursor-pointer"
                                                        >
                                                            Reject
                                                        </button>
                                                    </div>
                                                ...
                                                {order.deliveryStatus === 'Accepted' && (
                                                    (order.status === 'Ready' || order.status === 'Ready for Pickup' || order.status === 'Out for Delivery') ? (
                                                        ...
                                                        <div className="text-center p-3.5 bg-slate-950/40 border border-slate-850 rounded-2xl text-xs font-black text-slate-500 tracking-wide flex items-center justify-center gap-1.5">
                                                            <Clock size={12} className="animate-spin text-slate-600" /> Kitchen is preparing order...
                                                        </div>
                                                    )
                                                )}
                        ...
                        <div className="lg:col-span-1 space-y-6">
                            {/* Shift Summary Cards */}
                            <div className="bg-slate-900/60 backdrop-blur-md p-6 rounded-[2.5rem] border border-slate-800/80 shadow-xl space-y-4 text-left">
                                <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest">Shift Summary</h3>
                                <div className="space-y-3.5 pt-2">
                                    <div className="flex justify-between items-center text-xs">
                                        ...
                                        <span className={`px-2 py-0.5 rounded-full font-black text-[9px] uppercase ${
                                            profile?.status === 'Online' ? 'bg-emerald-500/10 text-emerald-400' : 'bg-slate-850 text-slate-550'
                                        }`}>{profile?.status}</span>
                                    </div>
                                    <div className="flex justify-between items-center text-xs">
                                        <span className="text-slate-500">Registered Vehicle</span>
                                        <span className="text-white font-bold">{profile?.vehicleDetails?.type || 'Bike'}</span>
                                    </div>
                                    <div className="flex justify-between items-center text-xs">
                                        <span className="text-slate-500">Current Wallet Balance</span>
                                        <span className="text-emerald-400 font-black">₹{earnings?.stats?.walletBalance || 0}</span>
                                    </div>
                                    <div className="flex justify-between items-center text-xs">
                                        <span className="text-slate-500">Total Deliveries Completed</span>
                                        <span className="text-white font-black">{earnings?.stats?.totalDeliveries || 0}</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="bg-slate-900/60 backdrop-blur-md p-6 rounded-[2.5rem] border border-slate-800/80 shadow-xl space-y-4 text-left">
                                <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest">Rider guidelines</h3>
                ...
                {activeSection === 'earnings' && (
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
                        {/* Stats & Actions */}
                        <div className="lg:col-span-1 space-y-6">
                            <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest pl-1 text-left">Metrics</h3>
                            
                            <div className="grid grid-cols-2 lg:grid-cols-1 gap-4">
                                <div className="bg-slate-900/60 backdrop-blur-md border border-slate-800/80 p-5 rounded-[2.2rem] flex items-center gap-3.5 shadow-xl text-left">
                                    ...
                                    <div>
                                        ...
                                        <h3 className="text-lg font-black text-white mt-1">₹{earnings?.stats?.walletBalance || 0}</h3>
                                    </div>
                                </div>
                                <div className="bg-slate-900/60 backdrop-blur-md border border-slate-800/80 p-5 rounded-[2.2rem] flex items-center gap-3.5 shadow-xl text-left">
                                    <div className="p-3.5 rounded-2xl bg-teal-500/10 text-teal-400 border border-teal-500/20 shadow-inner">
                                        <DollarSign size={20} />
                                    </div>
                                    <div>
                                        <span className="text-[10px] text-slate-500 font-black block uppercase tracking-wider font-sans">Earnings</span>
                                        <h3 className="text-lg font-black text-white mt-1">₹{earnings?.stats?.totalEarnings || 0}</h3>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="lg:col-span-2 space-y-4">
                            <div className="flex justify-between items-center px-1">
                                <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest">Delivery History</h3>
                                <span className="text-[9px] bg-slate-900 border border-slate-800 px-2 py-0.5 rounded-full text-slate-400 font-bold">{earnings?.history?.length || 0} Runs Completed</span>
                            </div>
                            
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-h-[500px] overflow-y-auto pr-1.5 custom-scrollbar">
                                {earnings?.history?.length === 0 ? (
                                    <div className="col-span-2 py-12 bg-slate-900/40 border border-slate-850 rounded-[2rem] text-center text-slate-500 font-bold text-xs">
                                        No completed deliveries yet.
                                    </div>
                                ) : (
                                    earnings?.history?.map(h => (
                                        <div key={h._id} className="p-4.5 bg-slate-900/60 rounded-2xl border border-slate-800/80 flex justify-between items-center hover:border-slate-700 transition-colors">
                                            <div className="space-y-1 text-left">
                                                <div className="flex items-center gap-1.5">
                                                    <span className="w-1.5 h-1.5 rounded-full bg-emerald-500"></span>
                                                    <p className="font-extrabold text-white text-xs">Order #{h._id.substring(h._id.length - 4).toUpperCase()}</p>
                                                </div>
                                                <p className="text-[9px] text-slate-500 font-medium pl-3">{new Date(h.updatedAt).toLocaleString()}</p>
                                            </div>
                                            <div className="text-right">
                                                <span className="font-black text-emerald-400 text-sm">₹{h.deliveryCharge}</span>
                                                <p className="text-[8px] font-black text-slate-500 uppercase tracking-wider mt-0.5">Paid</p>
                                            </div>
                                        </div>
                                    ))
                                )}
                ...
                {activeSection === 'profile' && profile && (
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
                        {/* Profile Info */}
                        <div className="lg:col-span-1 bg-slate-900/60 backdrop-blur-md border border-slate-800/80 rounded-[2.5rem] p-6 shadow-xl space-y-6">
                            {/* Profile Header */}
                            <div className="flex flex-col items-center text-center space-y-3.5 pb-6 border-b border-slate-800/60">
                                <div className="w-20 h-20 rounded-full bg-gradient-to-tr from-emerald-500/10 to-teal-400/10 text-emerald-400 border border-emerald-500/20 flex items-center justify-center font-black text-2xl shadow-xl">
                                    {profile.userId?.name?.slice(0, 2).toUpperCase()}
                                </div>
                                <div className="space-y-1">
                                    <h3 className="text-lg font-black text-white leading-none">{profile.userId?.name}</h3>
                                    <p className="text-[10px] text-slate-500 font-bold uppercase tracking-wider mt-1.5">Verification Status</p>
                                    <span className="inline-block px-2.5 py-0.5 rounded-full text-[9px] font-black bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 uppercase tracking-widest">
                                        Approved Agent
                                    </span>
                                </div>
                            </div>
                            ...
                            {/* Verification Stats */}
                            <div className="space-y-4 text-xs font-semibold text-slate-350 text-left">
                                <div className="p-4 bg-slate-950/40 border border-slate-850 rounded-2xl flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-sm font-bold text-white mt-0.5">+91 {profile.userId?.phoneNumber || '—'}</p>
                                    </div>
                                </div>

                                <div className="p-4 bg-slate-950/40 border border-slate-850 rounded-2xl flex items-center gap-3">
                                    <Truck className="text-emerald-400 shrink-0" size={18} />
                                    <div>
                                        <span className="text-[9px] text-slate-500 uppercase tracking-wider block font-bold">Vehicle Details</span>
                                        <p className="text-sm font-bold text-white mt-0.5">{profile.vehicleDetails?.type || 'Bike'} · {profile.vehicleDetails?.model || 'Activa CG'}</p>
                                    </div>
                                </div>
                                ...
                                <div className="p-4 bg-slate-950/40 border border-slate-850 rounded-2xl flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-sm font-bold text-white mt-0.5 font-mono">{profile.vehicleDetails?.licenseNumber || 'DL-XXXX'}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="lg:col-span-2 space-y-6">
                            <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest pl-1 text-left">Performance Overview</h3>
                            
                            <div className="bg-slate-900/60 backdrop-blur-md border border-slate-800/80 rounded-[2.5rem] p-6 shadow-xl space-y-6">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-left">
                                    <div className="p-5 bg-slate-950/40 border border-slate-850 rounded-3xl space-y-1">
                                        ...
                                        <h4 className="text-2xl font-black text-emerald-400">99.2%</h4>
                                    ...
                                    <div className="p-5 bg-slate-950/40 border border-slate-850 rounded-3xl space-y-1">
                                        ...
                                        <h4 className="text-2xl font-black text-teal-400">4.9 ★</h4>
                                        <p className="text-[9px] text-slate-500">Excellent feedback on behavior & timeliness</p>
                                    </div>
                                </div>

                                <div className="p-5 bg-slate-950/40 border border-slate-850 rounded-3xl text-left space-y-2">
                                    <h4 className="text-xs font-black text-slate-350 uppercase tracking-wider">System notifications</h4>
            ...
            <nav className="fixed bottom-0 inset-x-0 bg-slate-900/90 backdrop-blur-md border-t border-slate-800/80 py-3 px-6 flex justify-around items-center z-20 shadow-[0_-10px_35px_rgba(0,0,0,0.6)]">
                {[
            ...
            {isWithdrawOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-md animate-in fade-in duration-200">
                    <div className="bg-slate-900 border border-slate-800 p-6 rounded-[2.5rem] shadow-2xl w-full max-w-sm relative space-y-4">
                        <h3 className="font-black text-white text-base">Request Payout Withdrawal</h3>
                        <form onSubmit={handleRequestWithdrawal} className="space-y-4 text-left">
                            ...
                            <div className="flex gap-3 pt-4">
                                <button
                                    ...
                                    className="flex-1 py-3 bg-slate-850 hover:bg-slate-800 text-slate-300 font-bold rounded-xl text-xs transition-colors cursor-pointer"
                                >
                                    Cancel
                                </button>
                                <button
            ...
            {showNavigationModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-md animate-in fade-in duration-200">
                    <div className="bg-slate-900 border border-slate-800 p-6 rounded-[2.5rem] shadow-2xl w-full max-w-sm relative text-center space-y-5">
                        ...
                        <div className="flex justify-between items-center bg-slate-950/60 p-3 rounded-2xl border border-slate-850">
                            <div className="flex items-center gap-2 text-emerald-400 font-extrabold text-[10px] uppercase tracking-widest">
                                <Navigation size={14} className={isSimulating ? "animate-bounce text-emerald-400" : "text-slate-500"} />
                                <span className={isSimulating ? "text-emerald-400" : "text-slate-500"}>
                                    {isSimulating ? "GPS Tracking Active" : "GPS Signal Idle"}
                                </span>
                            </div>
                            <span className="text-[10px] bg-slate-800 px-2 py-0.5 rounded font-black text-slate-400 uppercase tracking-widest font-mono">
                        ...
                        <div className="space-y-2">
                            ...
                            {isSimulating && (
                                <div className="py-2 text-[10px] text-emerald-400 font-bold animate-pulse uppercase tracking-widest font-black">
                            ...
                            <button
                                ...
                                className="w-full py-3 bg-slate-850 hover:bg-slate-800 border border-slate-750 text-slate-200 font-black rounded-xl text-xs uppercase tracking-wider transition-colors cursor-pointer"
                            >
                                Close Directions
                            </button>
                        </div>
